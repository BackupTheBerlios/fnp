...
object img =Image.load("rv/germany.map");
  ...
  img=img->paste_alpha_color(Image.Font()->write(from),0,0,255,Y1+4,X1+4 );
  img=img->paste_alpha_color(Image.Font()->write(to),0,0,255,Y2+4,X2+4 );

  img=img->paste_alpha_color(Image.Font()->write(time_now()->date+" "+time_now()->time),0,0,0,1,1 );
  img=img->paste_alpha_color(Image.Font()->write(from + "-" + to),0,0,0,1,12 );
